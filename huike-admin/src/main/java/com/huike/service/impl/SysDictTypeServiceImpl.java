...
public class SysDictTypeServiceImpl implements ISysDictTypeService {
    ...
    @PostConstruct
    public void init() {
        log.debug("初始化时, 加载数据字典到redis缓存中...【开始】");
        List<SysDictType> dictTypeList = dictTypeMapper.selectDictTypeAll();
        for (SysDictType dictType : dictTypeList) {
            List<SysDictData> dictDatas = dictDataMapper.selectDictDataByType(dictType.getDictType());
            DictUtils.setDictCache(dictType.getDictType(), dictDatas);
        }
        log.debug("初始化时, 加载数据字典到redis缓存完毕...【完毕】");
    }
    ...
    public int deleteDictTypeByIds(Long[] dictIds) {
        ...
        if (count > 0) {
            DictUtils.clearDictCacheWithTypes(dictTypes.toArray(new String[dictTypes.size()]));
        }
        return count;
    }
    ...
    public int updateDictType(SysDictType dictType) {
        ...
        if (!oldDict.getDictType().equals(dictType.getDictType())) { //字典类型修改前后不一致
            dictDataMapper.updateDictDataType(oldDict.getDictType(), dictType.getDictType());
        }
        int row = dictTypeMapper.updateDictType(dictType);
        if (row > 0) {
            DictUtils.clearDictCacheWithTypes(oldDict.getDictType(), dictType.getDictType());
        }
        return row;
    }
