...
public class ReportServiceImpl implements IReportService {
    ...
    @Override
    public Map<String, Object> getcontractsBasicInfo(IndexStatisticsVo indexStatisticsVo, String now) {
        Map<String, Object> result = new HashMap<String, Object>();
        String username = CurrentUserHolder.getUserName();
        try {
            CompletableFuture<Map<String, Object>> baseData = CompletableFuture.supplyAsync(() -> {
                //处理基础数据
                return reportMpper.getBaseData(indexStatisticsVo, now, username);
            });
            CompletableFuture<Map<String, Object>> todayData = CompletableFuture.supplyAsync(() -> {
                //处理今日简报
                return reportMpper.getTodayData(indexStatisticsVo, now, username);
            });

            CompletableFuture<Map<String, Object>> todoData = CompletableFuture.supplyAsync(() -> {
                //处理待办事项
                return reportMpper.getTodoData(indexStatisticsVo, now, username);
            });
            CompletableFuture.allOf(baseData, todayData, todoData).join();
